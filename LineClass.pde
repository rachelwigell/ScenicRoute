...
public class Line {
  ...
  public void drawSolid(){
    Vector3D col = colorVal(this.start.z/10+100);
    fill((int) col.x, (int) col.y, (int) col.z);
    noStroke();
    this.start.x -= 5;
    this.end.x -= 5;
  ...
  public void drawLine(){
    ...
    this.start.x -= 5;
    this.end.x -= 5;
    strokeWeight(end.squareDistance(start)/1000);
  ...
  public Vector3D colorVal(float zCoord){
    return new Vector3D(280-zCoord, 250-zCoord, 330-1.6*zCoord);
  }
}