public class Deciduous {
  ...
  public Deciduous(Vector3D position, int numRoots, int numBranches, int endLevel){
    this.roots = new Deciduous[numRoots];
    this.numRoots = numRoots;
    this.numBranches = numBranches;
    this.endLevel = endLevel;
    this.roots[0] = new Deciduous(position, numRoots, numBranches, endLevel, 0, 0);
    for(int i = 1; i < numRoots; i++){
        this.roots[i] = new Deciduous(this.roots[i-1].path.end, numRoots, numBranches, endLevel, i, this.roots[i-1].path.endFace.radius);
    }
  }
  
  public Deciduous(Vector3D position, int numRoots, int numBranches, int endLevel, int rootLevel, float radius){
    float yDist = random((.4*screenY)/(numRoots*sqrt(rootLevel+2)), (.6*screenY)/(numRoots*sqrt(rootLevel+2)));
    ...
    this.numRoots = numRoots;
    this.branches = new Deciduous[numBranches];
    for(int j = 0; j < numBranches; j++){
      branches[j] = new Deciduous(this);
    }
  }
  
  public Deciduous(Deciduous root){
    this.numRoots = root.numRoots;
    this.level = root.level+1;
    this.numBranches = root.numBranches-1;
    this.endLevel = root.endLevel;
    this.roots = new Deciduous[this.numRoots];
    this.roots[0] = new Deciduous(root, true, null, 0, true);
    for(int i = 1; i < this.numRoots; i++){
      this.roots[i] = new Deciduous(root, false, this.roots[i-1].path, i, true);
    }
  }
  
  public Deciduous(Deciduous root, boolean first, Line prevPath, int rootLevel, boolean differentiate){
    this.rootLevel = rootLevel;
    this.numRoots = root.numRoots;
    this.level = root.level+1;
    this.numBranches = root.numBranches-1;
    ...
    if(this.level < this.endLevel){
      ...
      for(int i = 0; i < numBranches; i++){
        branches[i] = new Deciduous(this);
      }
    }
  }
  
  public void drawRoot(String style){
    for(Deciduous d: this.roots){
      d.drawBranch(style);
    }
  }
